...
impl SemanticNameIndex {
    ...
    pub fn empty_delta(&self) -> SemanticNameDelta {
        SemanticNameDelta::new(self.snapshot_id, self.next_node_id, self.next_symbol_id)
    }

    pub fn capability(&self) -> SemanticNameResolver<'_> {
        SemanticNameResolver { snapshot: self }
    }
    ...
    /// All namespace nodes of this snapshot, in id order.  Used by the
    /// semantic world to close the owner registration over an adopted
    /// bootstrap index in one atomic step.
    pub fn namespace_nodes(&self) -> impl Iterator<Item = &NamespaceNode> {
        self.nodes.values()
    }
    ...
    ) -> Result<SemanticNameIndex, SemanticNameInstallError> {
    ...
    fn validate_delta(&self, delta: &SemanticNameDelta) -> Result<(), SemanticNameInstallError> {
        if delta.base_snapshot_id != self.snapshot_id {
            return Err(SemanticNameInstallError::single(Diagnostic::hard_error(
        ...
        if diagnostics.is_empty() {
            ...
            Err(SemanticNameInstallError { diagnostics })
        }
    }
}

impl Default for SemanticNameIndex {
    fn default() -> Self {
        Self::new()
    }
}
...
pub struct SemanticNameResolver<'snapshot> {
    pub(crate) snapshot: &'snapshot SemanticNameIndex,
}

impl<'snapshot> SemanticNameResolver<'snapshot> {
    ...
    pub fn open_virtual_node(
        ...
        source_category: SourceCategory,
        provenance: Provenance,
    ) -> (NamespaceNodeId, SemanticNameDelta) {
        let mut delta = self.snapshot.empty_delta();
        let key = key.into();
...
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct SemanticNameInstallError {
    pub diagnostics: Vec<Diagnostic>,
}

impl SemanticNameInstallError {
...
impl From<SemanticNameInstallError> for BuildError {
    fn from(error: SemanticNameInstallError) -> Self {
        Self {
