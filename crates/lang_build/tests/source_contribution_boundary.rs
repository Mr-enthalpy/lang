...
fn type_value_binding_placeholder_keeps_fresh_symbol_place_until_type_values_exist() {
    ...
    // first-order projection material and writable-place checking exist.
    ...
    // The placeholder `TypeObject` exists only because type-object binding
    // evaluation and writable-place checking are not yet implemented.
    // When those features land, this test must be replaced with a proper
    // type-object binding test (fresh symbol/place `T` bound to existing type
    // object `uint8`, place(T) != place(uint8)).
    //
    // v0.6 placeholder behavior only: this is not final type-object binding semantics.
    // Long-term, `let T: type = uint8` binds fresh symbol/place `T` to the
    // existing type object `uint8`; injection through `T` targets place(T), not
    // place(uint8).
