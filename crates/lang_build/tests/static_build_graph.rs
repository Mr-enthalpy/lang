...
// A. Build a single package through BuildSession from a committed fixture.
#[test]
fn builds_single_package_through_build_session() {
    let workspace = single_package_fixture("single_package_type_binding", "app");
...
// B. Cache hit on a repeated build with the same session and fixture workspace.
#[test]
...
fn cache_miss_when_source_content_changes() {
    ...
    let workspace = BuildWorkspace {
        packages: vec![package_spec("app", temp.path().join("app/src"))],
    };
...
fn static_dependency_order_is_deterministic() {
    let workspace = static_dependency_chain_fixture();
    let mut session = BuildSession::new();
    let result = session
        .build_workspace(&workspace)
        .expect("build workspace");
...
// E. Unknown dependency is a hard error (validation-only; no source discovery).
#[test]
fn unknown_dependency_is_a_hard_error() {
    let mut app = bare_package_spec("app");
...
// F. Dependency cycle is a hard error and nothing builds (validation-only).
#[test]
fn dependency_cycle_is_a_hard_error() {
    let mut a = bare_package_spec("a");
    a.dependencies.push(dependency("b", &["b"]));
    let mut b = bare_package_spec("b");
...
// G. Duplicate package names are a hard error (validation-only).
#[test]
fn duplicate_package_names_are_a_hard_error() {
    let workspace = BuildWorkspace {
        packages: vec![bare_package_spec("app"), bare_package_spec("app")],
    };
...
// H. Self-dependency is a hard error (validation-only).
#[test]
...
fn dependency_fingerprint_participates_in_dependent_cache_key() {
    ...
    let dep = package_spec("dep", temp.path().join("dep/src"));
    let mut app = package_spec("app", temp.path().join("app/src"));
    app.dependencies.push(dependency("dep", &["dep"]));
    let workspace = BuildWorkspace {
...
// are unchanged. Source comes from a committed fixture; only the API-level mount
// varies (cache-invalidation test).
#[test]
...
// CompilationWorld::from_manifest (validation-only; bare specs).
#[test]
fn explicit_mount_and_dependency_sharing_mount_path_fail_at_validation() {
    ...
    app.dependencies.push(dependency("dep", &["dep"]));
    let workspace = BuildWorkspace {
        packages: vec![bare_package_spec("dep"), app],
    };
