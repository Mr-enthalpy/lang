...
| Pipe branch-name shorthand (`|> name { ... }`) | `implemented-syntax` | `pipe.rs` | `ast-construction-v0.1.md` §7.1.1 | `v0.1.w` additive local sugar and narrow repair for the otherwise-invalid local shape that would fall toward right-call composition into a headless in-place closure. Accepted only as a mechanical shorthand for `|> (_ name) { ... }`; following token sequences are parsed by ordinary pipe / segment / composition rules. Not a precedent for a family of branch-arm sugars. No semantic validation, matching, lookup, or closure materialization. |
