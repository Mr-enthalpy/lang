...
| `obj[args...]` bracket-call sugar | `implemented-syntax` | `atom.rs`, `operator.rs`, `argpack.rs` | `ast-construction-v0.1.md` §8.7 | `BracketCallSugar` node (atom + operator layer); operator spelling `[]`. Source-preserving; no indexing/slicing/container semantics, no lowering. `obj[]` valid. |
...
| Argpack comma→unit slots | `implemented-syntax` | `argpack.rs`, `ast.rs` | `ast-construction-v0.1.md` §9.1 | Commas create slots; empty slot preserved as `ExprKind::Unit`; zero slots without commas is empty. Applies to paren and bracket argpacks. Source-preserving. |
