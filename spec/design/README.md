...
| Symbol-first resolution, complete type projection, `compile` / `meta`, `struct -> symbol`, pure `extend`, place-level `inject`, binding/install boundary | `symbol-world/symbol-first-meta-construction-and-pattern-injection.md` | A consistency summary, implementation gap, and link |
