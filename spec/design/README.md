...
| Abstract literal denotations, ordinary construction to concrete machine-semantic Types, same-Type materialization, and stage-invariant Type semantics | `symbol-world/type-values-places-and-borrow-views.md` | Current LiteralFamily/T/Tnum helper/core coverage stays in the v0.6 transition contract as an implementation subset |
...
| Continuation-relative `@`, LifeName/LifetimeValue/NameView, Region/generation/cleanup, Pre/Post summaries, origin/Color, and lifetime separation from type/compile overload | `lifetime/lifetime-policy-and-overload-boundary.md` | Concrete IR, checker, summary compression, access-tree integration, and extended Horae logic |
...
| `lifetime/` | Canonical owner of continuation-relative `@`, LifeName/LifetimeValue/NameView, Region/generation/cleanup, Pre/Post summaries, lazy/coinductive origin, Color inheritance, escape checking, and the post-overload boundary. `ref` / `share` alone retain `PrivilegedActualPlace`; `@` is not a borrow or place-acquisition operation. | Concrete IR, lifetime checker, summary compression, access-tree integration, diagnostics, and extended Horae logic. |
