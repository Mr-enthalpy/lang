...
| Canonical `Pv:Pp`, orthogonal whole-slot `PolicyMode`, producer/destination mode separation with mechanical transfer, contextual binding `P1`, result `P2`, function-object stage derivation, seal visibility/snapshot, three-point preference, 3×3 capability realization, stable export facts plus consumer-context capability, no scalar replacement for `Pv:Pp` and no independent P3, mechanical compile projection, companions, match staging, and coarse automatic require | `symbol-world/symbol-policy-and-compile-flow-projection.md` | Implementation mapping, invocation handoff, pattern algebra, and links |
...
| Continuation-relative `@`, LifeName/LifetimeValue/NameView, pairwise-distinct exclusive and same-root shared defaults plus finite Pre patch, exact move-origin law, gapless half-open Region generations, selected CopyConstruct lifecycle posts, finite Color relations, and lifetime separation from type/compile overload | `lifetime/lifetime-policy-and-overload-boundary.md` | Concrete IR, checker, summary compression, access-tree integration, and extended Horae logic |
...
| `lifetime/` | Canonical owner of continuation-relative `@`, LifeName/LifetimeValue/NameView, pairwise-distinct exclusive and same-root shared defaults plus finite Pre patch, move-origin preservation, gapless Region generations, selected CopyConstruct lifecycle posts, Pre/Post summaries, finite/monotone Color relations, escape checking, and the post-overload boundary. `ref` / `share` alone retain `PrivilegedActualPlace`; `@` is not a borrow or place-acquisition operation. | Concrete IR, lifetime checker, summary compression, access-tree integration, diagnostics, and extended Horae logic. |
