...
| Existing-view-first demand satisfaction, binding-P1 conservative-extension corollary, atomic Runtime Val1 migration, and mixed-stage boundary | `symbol-world/symbol-policy-and-compile-flow-projection.md` | Current binding-P1 helper coverage stays in the v0.6 transition contract; complete mixed-stage evaluation stays in planning |
| Atomic migration as compiler-inserted ordinary function-object invocation, distinct from structure-changing mechanical operations | `symbol-world/function-object-call-model.md` | Current caller-supplied callable/result carriers are fixtures, not the final route or a new callable ontology |
| Transition endpoint Policy preference placement in the overload pipeline | `patterns-overload/overload-resolution-design.md` | Input × output endpoint fit extends Bp before Pattern specificity; it is not a B6 named strategy |
...
| Current cross-Policy and literal helper implementation subset | `../contracts/v0.6-cross-policy-value-transition.md` | Records implemented carriers and deferred integration only; does not own language semantics |
