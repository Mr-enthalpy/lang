...
| Canonical `Pv:Pp`, orthogonal whole-slot `PolicyMode`, contextual binding `P1`, result `P2`, function-object stage derivation, seal visibility/snapshot, three-point preference, 3×3 capability realization, stable external projection plus later consumer checks, no scalar result policy, mechanical compile projection, companions, match staging, and coarse automatic require | `symbol-world/symbol-policy-and-compile-flow-projection.md` | Implementation mapping, invocation handoff, pattern algebra, and links |
...
| Continuation-relative `@`, LifeName/LifetimeValue/NameView, default coinductive origin universe plus finite Pre patch, exact move-origin law, Region/generation/cleanup, finite Color relations, and lifetime separation from type/compile overload | `lifetime/lifetime-policy-and-overload-boundary.md` | Concrete IR, checker, summary compression, access-tree integration, and extended Horae logic |
...
| `lifetime/` | Canonical owner of continuation-relative `@`, LifeName/LifetimeValue/NameView, default coinductive origin universe plus finite Pre patch, move-origin preservation, Region/generation/cleanup, Pre/Post summaries, finite/monotone Color relations, escape checking, and the post-overload boundary. `ref` / `share` alone retain `PrivilegedActualPlace`; `@` is not a borrow or place-acquisition operation. | Concrete IR, lifetime checker, summary compression, access-tree integration, diagnostics, and extended Horae logic. |
