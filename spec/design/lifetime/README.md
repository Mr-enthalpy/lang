...
`share` are the borrow constructors; they belong to the same privileged builtin
callable family and may obtain the actual's place, while an ordinary user
function cannot. Explicit higher-level selection uses `t |> type ref` /
