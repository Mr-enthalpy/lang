...
There is no source-level syntax such as `import mylib`, `use mylib::math`,
...
After this, source code may refer to `std::Vec` and `mylib::math::vector::Vec3`
...
Consider `((int)std::Vec)::ns1`:

- `std` may be the last filesystem-backed physical namespace layer.
- `std::Vec` is a declaration found inside that physical namespace.
- `(int)std::Vec` is an instantiated node. It does not correspond to a folder.
- `((int)std::Vec)::ns1` is a virtual child namespace under the instantiated
...
Source code should normally write `mylib::math::Vec3`, not
`mylib_1_2_0::math::Vec3`. Version selection belongs to the build/package
layer.
...
v0.1 should only preserve raw syntax such as `std::Vec`,
`mylib::math::vector::Vec3`, `((int)std::Vec)::ns1` to the extent such syntax
