...
| Symbol | `TokenKind::Symbol(Symbol)` (19 variants: `LParen`, `RParen`, `LBracket`, `RBracket`, `LBrace`, `RBrace`, `Comma`, `Colon`, `Equal`, `Dot`, `DotDot`, `ColonColon`, `PipeGreater`, `FatArrow`, `ThinArrow`, `Less`, `Greater`, `Semicolon`, `TripleEqual`) | `token.rs`, `lexer.rs` | `ast-construction-v0.1.md` §2 | Each structural symbol is a single token with maximal-munch priority. `TripleEqual` is a structural delimiter, not an operator spelling. | The lexer distinguishes structural symbols from operator spellings. `Less` and `Greater` are bare symbols that the parser may reinterpret as operator spellings in expression contexts. | v0.3 may rely on symbol identity for structural parsing. | v0.3 must not reinterpret `TripleEqual` as an operator or an equality comparison. |
| Operator | `TokenKind::Operator(OperatorSpelling)` (33 variants: 32 lexer-produced + 1 contextual `BracketCall`) | `token.rs`, `lexer.rs` | `operator-design.md` | 32 lexer-produced operator spellings recognized via maximal-munch. `BracketCall` (`[]`) is a contextual paired operator name (never produced by the lexer as a single token). | Operator spellings are syntax-level names. They do not imply built-in arithmetic, comparison, mutation, or lookup. | v0.3 may preserve operator spellings through normalization. | v0.3 must not perform operator lookup, overload resolution, ADL, or type-directed operator selection. |
...
| AST / token shape | `TokenKind::Operator(OperatorSpelling)` (33 variants) |
| Implementation file(s) | `token.rs`, `lexer.rs` |
| Spec source | `operator-design.md` |
| Frozen guarantee | 32 lexer-produced operator spellings recognized via longest-match. `BracketCall` (`[]`) is a contextual paired operator name, never produced by the lexer as a single token, used as the operator identity in bracket-call sugar and bindable/aliasable in operator-name positions. `TripleEqual` (`===`) is `Symbol::TripleEqual`, not an operator spelling. |
...
| Forbidden assumption | v0.3 must not perform operator lookup, overload resolution, or ADL. v0.3 must not treat `TripleEqual` as an operator or equality comparison. |
