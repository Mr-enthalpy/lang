...
| Symbol | `TokenKind::Symbol(Symbol)` (20 variants, including `Dot`, `DotDot`, and `Ellipsis`) | `token.rs`, `lexer.rs` | `ast-construction-v0.1.md` §2 | Each structural symbol is a single token with maximal-munch priority. `...` is recognized before `..`/`.`. `TripleEqual` is a structural delimiter for alias-let at the lexer/form boundary. | The lexer distinguishes structural symbols from lexer-produced operator tokens and assigns no pack/spread semantics to `Ellipsis`. | v0.3 may rely on symbol identity for structural parsing. | v0.3 must not treat parser-reinterpreted `TripleEqual` as semantic equality, alias resolution, or forwarding, or treat `Ellipsis` as an RHS spread. |
...
| AST / token shape | `BindingSlotAst { policy: Option<PolicySpecAst>, has_let: bool, deduce: Option<DeduceListAst>, pattern: BindingPatternAst, annotation: Option<BindingAnnotationAst>, with_clause: Option<WithClauseAst>, initializer: Option<ExprAst>, span: Span }`; `PolicySpecAst { value_policy: ValuePolicyPatternAst, pattern_policy: Option<PolicyConjunctionAst>, span }`; dedicated `PolicyConjunctionAst`, `PolicyChoiceAst`, and `PolicyAtomAst` nodes |
...
| Frozen guarantee | The binding slot shape is reused across let bindings, parameter slots, and return slots. Optional `policy` is recognized only by the `PolicySpec let` syntactic shape; the strong-context parser preserves `||` choice, `+` conjunction, and optional value/Pattern `:` pair structure. Single `|` remains Pattern alternative. `policy = None` means unwritten (implicit/inferred later). `AbsentValuePattern` reserves empty-value capability without freezing a final source token. Initializer is required only in let-binding context. `with { ... }` is rejected in return slots. |
...
| AST / token shape | `BindingPatternAst::Binder(BinderNameAst)`, `BindingPatternAst::Product(ProductExtractAst)`, `BindingPatternAst::Pack { inner, span }`, `BindingPatternAst::Skeleton(CanonicalSkeletonAst)`, `BindingPatternAst::Error(ErrorAst)` |
| Implementation file(s) | `ast.rs`, `parser/let_stmt.rs`, `parser/canonical.rs` |
| Spec source | `ast-construction-v0.1.md` §4 |
| Frozen guarantee | Five pattern variants. `Pack` preserves `...Q`; each normalized structural level permits at most one pack, while nested levels are independent. |
...
| leading `.name` | `AtomKind::DotClosure { selector: SelectorAst }` | `ast.rs`, `parser/atom.rs` | callable-tail/dot/pack amendment | A leading dot plus text selector is an independent atom. | The parser performs no field lookup or type dispatch. | normalization generates the receiver-plus-remainder field-function closure; `E.name` routes through the same core. | normalization must not capture a receiver into `.name` itself. |
...
| Parser diagnostics (18) | Existing parser diagnostics plus `MultiplePackPatternsAtSameLevel` | `diagnostic.rs`, `parser/*.rs` | `diagnostics-v0.1.md` §3.2 + callable-tail/dot/pack amendment | Same-level pack duplication is diagnosed; `UnusedClosureAst` remains optional/not-guaranteed-emitted. |
| Return diagnostics (3) | `DiagnosticCode::ReturnRequiresValue`, `StatementAfterTerminalBlockForm`, `ReturnExpressionNotAllowed` | `diagnostic.rs`, `parser/form.rs` | `diagnostics-v0.1.md` §3.6 | Guaranteed: emitted for malformed return events — return without a value expression, statements placed after a terminal block form, and return expressions in non-return positions. |
...
| Field | Value |
|---|---|
| Frozen guarantee (general) | 33 `DiagnosticCode` variants (Lexer: 4, Parser: 18, Return: 3, Operator: 3, Alias: 5). Every diagnostic carries a `DiagnosticCode`, `message`, and `Span`. The parser is error-tolerant and retains the existing optional/reserved statuses. |
