...
| Binding-slot policy pair (`PolicySpec let …`) | `implemented-syntax` | `form.rs`, `let_stmt.rs`, `policy.rs`, `ast.rs`, `norm.rs` | `ast-construction-v0.1.md` §4.3 | Optional `PolicySpecAst` / `NormPolicySpec` preserved on `BindingSlotAst` and `LetAliasAst`. Supports one component or `value_policy:type_policy`; `+`/`|` stay ordinary operator-expression structure inside components. Recognized only by `PolicySpec let` in top-level, body, param, return, and alias positions. `policy = None` = unwritten/implicit. Contextual P1/P2 semantic validation belongs to `lang_build`; empty-value AST variant exists without a frozen source token. |
