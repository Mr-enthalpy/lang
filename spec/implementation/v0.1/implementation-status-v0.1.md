...
| leading `.name` dot closure | `implemented-normalization` | `atom.rs`, `ast.rs`, `norm.rs` | `callable-tail-dot-closure-and-pack-pattern.md` | Independent `DotClosure` atom; normalizes to receiver + `...args` field-function closure. `E.name` routes through the same core. |
...
| Pattern remainder `...Q` | `implemented-substrate` | `lexer.rs`, `let_stmt.rs`, `ast.rs`, `norm.rs`, `overload_pattern.rs`, `overload_set.rs` | `callable-tail-dot-closure-and-pack-pattern.md` | One pack per normalized level; restricted variadic applicability/binding and one-node specificity evidence. Not an ArgPack, value type, or RHS spread. |
...
| Callable implementation tail | `implemented-substrate` | `closure.rs`, `ast.rs`, `norm.rs`, `meta_body.rs`, `overload_set.rs` | `callable-tail-dot-closure-and-pack-pattern.md` | Supports ordinary/named body, `=> default`, bare/message delete, and no-`=>` `[[strategy]]` disambiguation. Named strategy metadata is carried after restricted applicability; arbitrary strategy execution/default generation remain future. |
