...
2. **type-associated namespace** (the companion space of a type object), or
3. **meta-function instantiation virtual layer**.

For a symbol `f::ns`: if `ns` is an ordinary (physical / merged) level, then `f`
is either a physical directory name or a type `f` (declared / merged across
files under `ns`) with its type-associated namespace — not both. If `ns` is
itself a meta-instantiation virtual layer, `f` is purely a virtual node
generated by that meta process.
