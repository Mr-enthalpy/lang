...
| 106-PT17 | Round-trip legal n through (n#)$ in the Path consumer | Interpret_Path yields p =_Path Read_name(n), then PathPatternProjection(p)=n#. General splice supplies no Path decoder and performs no resident read. | [Owner](../design/symbol-world/structured-path-algebra-and-pattern-splice.md) | Defined semantics; source/evaluator consumer pending |
