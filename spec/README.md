...
| `contracts/v0.6-cross-policy-value-transition.md` | Normative for the bounded implementation prototype | Records T/Tnum helpers, complete-choice projection followed by runtime-branch extraction, callable-owned migration mutability endpoints using ordinary actual-relative preference, pre-Bp fallback suppression, separation of complete ordinary result Policy from the demanded output view, and the private endpoint-ordering substrate without owning language semantics or claiming complete Bp'. |
