...
| `spec/design/symbol-world/` | Namespace graph world model: symbol-first facets, `SymbolId` / `PlaceId` / `PatternValue` / `TypeValueId`, alias forwarding, `compile` / `meta`, meta type self-root, resolved pattern scopes, `struct`, functional `inject`, namespace origin/construction ownership, binding/install, field functions, and the early-meta bootstrap. |
...
| `spec/design/meta-invocation/` | Policy-governed invocation: symbol-first heterogeneous value candidates, dual lookup/execution judgments, partial vs strict demand, residualization, guarded invocation, and control-like callables instead of `if constexpr` syntax. |
