...
| `v0.6-cross-policy-value-transition.md` | Normative for the current cross-Policy transition vertical slice | Keeps existing-slice projection separate from value transition, defines typed runtime failures and direct callable bridges, and freezes bidirectional Policy preference without a coercion graph. |
