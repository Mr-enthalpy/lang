...
This succeeds by selecting an ordinary source-declared `+` overload. A sum
result must be a real canonical sum-pattern value with the required
commutative, associative, and idempotent identity; otherwise evaluation returns
an explicit unsupported diagnostic. Meta bodies compute their declared result
through ordinary creation/write operations and a separate return event.
...
Resolved structural material uses stable Pattern roots and child evidence:

```text
DirectPatternChild(owner_root, "inner") -> field Pattern identity
```
...
Value-side lookup does not use structural child incidence. Completed paths
resolve a Symbol and then read its Pattern value; display names are not
semantic identity.
...
The struct decoder produces private syntax material consumed by `core::struct`.
The public result is the declared complete type value, never AST or decoder
material. Only outer binding/materialization installs namespace contributions.
