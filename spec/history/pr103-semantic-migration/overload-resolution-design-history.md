...
Default ordinary-meta result is `τ` (`DefaultMetaResult = τ`): `let` creates
members, `=` writes existing places, and the return event transfers control.
There is no alias-member event. See
`spec/design/symbol-world/symbol-first-meta-construction-and-pattern-injection.md`.
...
Compile-flow projection preserves ordinary calls; the static evaluator
distinguishes direct compile/meta/seal objects and derived companion objects.
...
manufactured. Migration can be reached only when the
...
An authorized atomic runtime migration routes an existing source
`PatternValue` to that owner's associated `()` callspace, then
through the same `PreparedCallCandidate`, `InvocationFrame`, and ordinary
selection path as source callables.
...
## 7. Connected invocation boundary
...
There is no maxima pass between ordinary and migration endpoint coordinates.
Without a migration context, endpoint coordinates are absent. Later filters
and operation-driven DynamicLegality premises extend this one pipeline; they
