...
overload `(self, t: type, _ unit: type): meta -> ...`. The left-hand `: type`
annotation is checked after the RHS result exists; it does not cause RHS meta
evaluation. Sum-pattern values require the canonical Pattern relation and must
not be approximated by policy union or operator reduction.

Ordinary `let` creates a Symbol/member, existing-place `=` writes, and the
return event delivers control. There is no alias-member event.
