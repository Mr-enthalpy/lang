...
namespace graph overload candidates
  -> Policy and Pattern selection
  -> unique sealed callable
  -> DynamicLegality
  -> execution
  -> InvocationResult
```
...
Selected delete and body failures produce `InvocationResult::Diagnostic` and
never cause candidate reselection. `Defaulted` remains a distinct
compiler-generation request until a callable-kind default generator exists.
...
Ordinary `let` creates a Symbol/member, ordinary `=` writes an existing place,
and the return event alone delivers control. A member that observes an external
object holds an ordinary borrow view (`ref` / `share`); borrow edges are not
owned material and are not promoted at seal. Installing an external pure Object
as the meta return role member fails the self-root invariant.
...
there is no private meta result ontology. The default meta result is a complete type value `τ`;
...
`τ = ⟨Q, V_τ⟩` independently owns its callspace, closes its dependencies, and
carries a stable meta-instance root. Primitive execution material remains
uninstalled until the outer result/binding boundary and defines no result class.
...
Primitive executors return replayable execution material, Residual, or
Diagnostic. Only after installation has formed the declared semantic entity may
the evaluator construct `InvocationResult::SemanticResult`.
...
```text
`struct`, verification, source callables, and migration families use one
ordinary invocation engine:
  resolve callee
  collect candidates
  match argument shape/pattern/type value
  select once under Policy preference
  validate DynamicLegality
  execute or residualize/diagnose
```

Primitive execution material is internal. `struct` forms an exact complete
type before a CompleteType semantic result crosses the invocation boundary;
outer binding owns namespace installation. Consumer gaps are tracked in the
roadmap rather than redefining this model.
...
`spec/contracts/meta-construction-boundary.md`.
