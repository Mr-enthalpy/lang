...
Owned type-field construction propagates `OpenCapability(child) => OpenCapability(parent)` and
`¬OpenCapability(parent) => ¬OpenCapability(child)`; mutability does not propagate.
