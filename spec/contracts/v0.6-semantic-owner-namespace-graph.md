...
`Pure(Q_struct)` and `TypeRole(Q_struct)`; structural leaves and
...
`V_τ`, and type projection returns the complete closure
`tau = <Q_struct,V_τ>`. Copied/extracted type-as-callee uses
`CallSpace(tau)=V_τ`; defining-Symbol or
