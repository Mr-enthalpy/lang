...
assignment/borrow partners are ordinary members of the returned Symbol's typed
buckets; same-name associated `Val2` Symbols expose those same members and own
no second copy. Members satisfying direct-home `TypeMember_Q_struct` enter
`V_T`, and type projection returns `CanonicalTypeObject(Q_struct,V_T)`.
Copied/extracted type-as-callee uses `CallSpace(T)=V_T`; defining-Symbol or
recent-carrier recovery is retired rather than deferred.
