...
`NoExternallyEligibleCandidate` is retained only as the typed name of the
current const-projected compatibility adapter. It is not the failure of the
stable namespace projection; future end-to-end resolution separates namespace
admission failures from later consumer capability/Policy failures.
