...
`struct` constructs the complete `Val1 × Pattern × Val2` object of its result
owner. Structural leaves and associated lets contribute to different axes of
...
An inner associated let adds a pending contribution to the uninstalled
`SymbolConstructionValue`; `struct` does not mutate the installed namespace
...
and installing `let name::name1::T = expr` after `T` construction produce the
same final namespace delta and candidate identity. The source navigation is
inner-to-outer.
