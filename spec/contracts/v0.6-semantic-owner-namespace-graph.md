...
installed as `()::ref::T`, for example, has its own callable owner while its
invocation receiver type is `ref::T`.
...
owner remains distinct from `ref::T`. If the installed call-entry owner and the
...
`struct` returns an ordinary Symbol PatternValue. Its unique type member owns
...
assignment/borrow partners occupy its associated Val2 and sibling callable
families.
...
and installing `let name::name1::((T |> type)@) = expr` after `T` construction
...
Writing local `let ()` while constructing `T` contributes only `()::T`. It does
not also create `()::ref::T` or `()::share::T`; those are distinct child-owner
injections and require their own contribution or future annotation rule.

Ordinary runtime-materializable fields generate runtime-or-compile accessors;
type/PatternValue fields use the same rule but are compile-only in this stage.
