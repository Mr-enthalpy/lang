...
- `ClosureAst` stores `placement: InPlace | Ordinary`,
  `head: Option<FnHeadPrefixAst>`, and `body: ClosureBodyAst` as independent
  dimensions.
- A bare `{ ... }` in atom position has `InPlace + head=None`; it is not a
  normal block expression.
- A headed no-`=>` block, including `[[strategy]]`, has
  `InPlace + head=Some`. `=>` selects `Ordinary + head=Some`.
- In-place closures cannot carry a capture list. The parser preserves such a
  form as Error rather than inventing a capture environment.
- A headless in-place closure has no extracted input. It is not equivalent to a
  ...
  implicit unit input.
- `ClosureBodyAst` is one of `Block`, `NamedBlock`, `Defaulted`, or
  `Delete(string_message?)`. `[[strategy]]` disambiguates named strategy
  metadata without stealing a return extraction pattern or changing placement.
- Invalid callable tails must remain Error nodes; they cannot recover as a
  legal empty `Block`.
