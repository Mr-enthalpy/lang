...
- Full lexical self-capability target resolution (not implemented; the current
  binder uses active frames and a temporary spelling-based self placeholder).
...
Do not claim that the restricted spelling-based binder is final self identity
resolution or that it performs result-slot injection.
